...
function activate(context) {
  let disposable = vscode.commands.registerCommand(
    "pupilfirst-translator.translate",
    async function () {
      ...
      if (editor) {
        ...
        try {
          const file = vscode.workspace
            .getConfiguration()
            .get("pupilfirst.translation.path");
          ...
          if (fileContents !== undefined) {
            ...
            if (keys !== undefined) {
              ...
              data["en"]["components"][fileName] = _.merge(
                data["en"]["components"][fileName],
                deepen(result)
              );
              // Replace selection with key
              editor.edit((builder) =>
                builder.replace(selection, 't("' + keys + '")')
              );
              // Write json to Yaml
            ...
            vscode.window.showInformationMessage(
              "Error: Please configure pupilfirst.translation.path in your settings"
            );
          }
        } catch (e) {
  ...
  let orderTranslation = vscode.commands.registerCommand(
    "pupilfirst-translator.reorder",
    async function () {
      const file = vscode.workspace
        .getConfiguration()
        .get("pupilfirst.translation.path");
